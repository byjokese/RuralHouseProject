...
public class SetAvailability2GUI extends JFrame {
	...
	private void jButton1_actionPerformed(ActionEvent e) {
		 ...
		 * try {
		 * 
		 * //It could be to trigger an exception if the introduced string is not a number float price= Float.parseFloat(jTextField3.getText());
		 * 
		 * //Obtain the business logic from a StartWindow class (local or remote) ApplicationFacadeInterface facade=StartWindow.getBusinessLogic();
		 * 
		 * //Offer o = facade.createOffer(ruralHouse, firstDay, lastDay, price); //System.out.println("Offer created: "+o.toString());
		 * 
		 * //if (o==null) // jLabel5.setText("Bad dates or there exists an overlapping offer"); // else jLabel5.setText("Offer created");
		 * 
		 * } catch (java.lang.NumberFormatException e1) { jLabel5.setText(jTextField3.getText()+ " is not a valid price"); } catch (OverlappingOfferExists e1) {
		 * jLabel5.setText("There exists an overlapping offer"); } catch (BadDates e1) { jLabel5.setText("Last day is before first day in the offer"); } catch
		 * (Exception e1) {
		 * 
		 * e1.printStackTrace(); }
		 */
